...
def scrape_lottery649(index):
    ...
    return Lottery649(
                ...
                due_date = soup.find(id=f"Lotto649Control_history_dlQuery_L649_EDate_{index}").string,
                sales_money = soup.find(id=f"Lotto649Control_history_dlQuery_L649_SellAmount_{index}").string,
                total_prize = soup.find(id=f"Lotto649Control_history_dlQuery_Total_{index}").string,
                prizes = __generate_lottery649_prizes(winners, prize_money_list, accumalted_list)
            )


def scrape_lottery649_lastest():
    return scrape_lottery649(0)
